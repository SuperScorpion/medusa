...
//        }
    ...
    public String sqlOfSelectCountCombo(Object[] objParams) {
        ...
        if (MedusaCommonUtils.isNotBlank(cacheSq)) {
            ...
            String countSq = cacheSq = cacheSq.replaceFirst("(?i)SELECT[\\s\\S]+?FROM", "SELECT COUNT(1) FROM")//量后面加? 实现非懒模式 modify by SuperScorpion on 20251205
                    .replaceAll("(?i)\\bORDER BY\\s+\\S+\\s+[ASC|DESC]*", "")
                    .replaceAll("(?i)\\bLIMIT ([0-9]*,[0-9]*|[0-9]*)", "");
